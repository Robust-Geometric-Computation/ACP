...
class UnitGradientV3 : public Object<PV3> {
  POLY3 p;
  PTR<Object<PV3>> v;

 public:
  UnitGradientV3(POLY3 p, PTR<Object<PV3>> v) : p(p), v(v) {}
  DeclareCalculate(PV3) {
    PV3<N> vp = v->get<N>();
    return p->get<N>().gradient(vp.x, vp.y, vp.z).unit();
  }
};
...
class P3toP3D : public Object<Poly3D> {
  POLY3 p;
  DeclareCalculate(Poly3D) {
    Poly3<N> p3 = p->get<N>();
    Poly3D<N> p3d(p3.xd, p3.yd, p3.zd);
    for (const pair<Poly3Inner::Index3, int> &ii : p3.ia)
      p3d.set(ii.first.i[0], ii.first.i[1], ii.first.i[2], p3.a[ii.second]);
    return p3d;
  }
public:
  P3toP3D (POLY3 p) : p(p) {}
...
void fregrid()
{
    ...
    Parameter boxlu = Parameter::constant(-1).interval(Parameter::constant(1));
    PV3<Parameter> box(boxlu, boxlu, boxlu);

    vector<PTR<Object<Poly3D>>> polys;
    for (int i = 0; i < drawer.poly3s.size(); ++i)
      polys.push_back(new P3toP3D(drawer.poly3s[i]));
    
    Encasement3D encasement_(box, polys);
    Encasement3D* encasement = &encasement_;
    ...
    for (int h0 = 0; h0 < drawer.poly3s.size(); h0++) {
      POLY3 poly3 = drawer.poly3s[h0];
      int h = h0 + 6;
	...
	for (const Mesh3D::Triangle &t : mesh)
	  for (int j = 0; j < 3; ++j) {
	    PV3<Parameter> pp = t.p[j]->getApprox();
	    vertex v;
	    v.x = pp.x.mid();
	    v.y = pp.y.mid();
	    v.z = pp.z.mid();

	    PTR<Object<PV3>> unitGrad = new UnitGradientV3(poly3, t.p[j]);
            PV3<Parameter> grad_unit = unitGrad->getApprox(1);
      ...
      drawer.vertex_all_vect.push_back(vertex_all);
    }
